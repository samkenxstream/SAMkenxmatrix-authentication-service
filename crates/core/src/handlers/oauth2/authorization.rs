...
use crate::{
    ...
    storage::{
        oauth2::{
            ...
            authorization_grant::{fulfill_grant, get_grant_by_id, new_authorization_grant},
...
async fn step(
    ...
    let reply = match browser_session.last_authentication {
        Some(Authentication { created_at, .. }) if created_at < grant.max_auth_time() => {
            let grant = fulfill_grant(&mut txn, grant, browser_session)
                .await
                .wrap_error()?;

            let session = match grant.stage {
                ...
                _ => {
                    return Err(anyhow::anyhow!(
                        "authorization grant not at the right stage"
                    ))
                    .wrap_error()
                }
            };
